...
class Checker {
    ...
    #apply(target, thisVal, [value]) {
        ...
        switch (true){
            case !!Object
                .keys(this.integrate)
                .filter((type)=> (target() || this.typeValue).toLowerCase().includes(type.toLowerCase()))
                .filter((integrated)=> {
                    this.error = false;
                    return this.integrate[integrated](value)
...
module.exports = { BaseInterface, Checker, JSON5Wrapper, MicroTest, Utility, aliasTypes, otherTypes, primitiveTypes, structuralTypes };
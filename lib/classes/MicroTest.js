import macro from "../../tests/macro.js";
...
export default class MicroTest {
    ...
    START = new Proxy(this, {
        get(target, name) {
            ...
            return (name)=> target.info.info(`${name}:`);
        }
    });
    ...
    failed = new Proxy(this, {
        get(target, name) {
            ...
            const stackArray = stack.split('at');
            const idx = stackArray[target.stackIdx] ? target.stackIdx: stackArray.length - 1;
            return (arg)=> target.info.error(` \u2717 ${name}.${arg} ->${stack.split('at')[idx].replace('\n','')}`);
        }
    });
